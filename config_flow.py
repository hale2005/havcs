...
from .const import  HAVCS_SERVICE_URL, DEVICE_PLATFORM_DICT, CONF_MODE, CONF_DEVICE_CONFIG, CONF_APP_KEY, CONF_APP_SECRET, CONF_BROKER, CONF_ENTITY_KEY, CONF_URL, CONF_PROXY_URL, CONF_SKIP_TEST, CONF_DISCOVERY, DEFAULT_DISCOVERY, INTEGRATION, CONF_HA_URL
    ...
    async def async_step_access(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if check:
                ...
                if self._mode == 1:
                    conf.update({
                        'http': {'clients':clients, 'ha_url': user_input[CONF_HA_URL]} if user_input[CONF_HA_URL] else  {'clients':clients},
                        'setting':{},
                    ...
                    conf.update({
                        ...
                        'http_proxy':{'ha_url': user_input[CONF_HA_URL]} if user_input[CONF_HA_URL] else {},
        ...
        if self._mode == 1:
            ...
            fields[vol.Optional(CONF_URL, default = user_input.get(CONF_URL, 'https://[你的公网域名或IP:端口号]'))] = str
            fields[vol.Optional(CONF_HA_URL, default = user_input.get(CONF_HA_URL, ''))] = str
        else:
